...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">
        <a href="{{ url('/admin/users') }}" class="nav-link"><i class="fas fa-cogs"></i> Permisos de Usuario: {{ $u->name.' '.$u->lastname}} (ID: {{ $u->id}})</a>
    </li>
@endsection
                                ...
                                <div class="inside">
                                    @foreach($value['keys'] as $k => $v)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="true" id="flexCheckDefault" name="{{ $k }}" @if(kvfj($u->permissions, $k)) checked @endif>
